...
## 🐛 Bug #17: 删除邮件逻辑存在 3 个缺陷

### 问题描述

删除邮件功能存在以下问题：

1. **🔴 高优先级：归档邮件无法在 IMAP 服务器上删除** - Worker 的 `deleteEmail` 方法只在 `INBOX` 中查找邮件，已归档的邮件删除会失败
2. **⚠️ 中优先级：API 错误时不提示用户** - 前端只处理成功情况，错误时不显示提示
3. **ℹ️ 低优先级：缺少 IMAP 删除结果反馈** - Worker 执行删除后不发送结果通知
...
- 用户在"归档"视图中删除邮件时，本地数据库记录被删除，但 IMAP 服务器上邮件仍保留
- 下次同步时邮件不会重新出现（因为 UID 已存在），但服务器存储空间未释放

**原因：**

```typescript
// worker/imap-worker.ts:511-534
async deleteEmail(uid: number): Promise<{...}> {
    const lock = await this.client.getMailboxLock('INBOX'); // ❌ 硬编码 INBOX
    // ...
}
```

**修复方案：**
需要从前端传递 `folder` 信息，或在数据库中查询邮件所在文件夹。
...
```typescript
// app/page.tsx:722-752
async function deleteEmail(emailId: string) {
    const r = await fetch(`/api/messages/${emailId}/`, { method: 'DELETE' });
    if (r.ok) {
        // 成功逻辑...
    }
    // ❌ 缺少 else 分支处理错误
...
- API 返回 404（邮件不存在）或 500 时，用户看不到任何提示
- 界面显示邮件已删除，但实际删除失败

#### 问题 3：缺少 WebSocket 反馈

**现状：**

- 前端发送 `{type:'delete', accountId, uid}` 到 Worker
- Worker 执行 IMAP 删除，成功或失败都不发送结果通知
- 前端无法确认 IMAP 删除是否成功
...
**方案 A（推荐）：数据库记录包含文件夹信息**

1. 修改 API 查询时返回 `folder` 字段
2. Worker 的 `deleteEmail` 接受 `folder` 参数
3. 前端 WebSocket 消息携带 `folder`
...
// API 返回
{
    ...
    folder: email.folder  // 新增
}

// Worker 方法
async deleteEmail(uid: number, folder: string): Promise<{...}> {
...
让 Worker 在所有常见文件夹（INBOX, Archive, SENT）中查找 UID 并删除。
...
if (r.ok) {
    ...
    setToastMessage(`❌ 删除失败: ${errorData.error || '未知错误'}`);
    setShowToast(true);
    setTimeout(() => setShowToast(false), 3000);
}
```

#### 修复 3：添加 WebSocket 反馈（可选）
...
if (data.type === 'delete_result') {
    if (!data.success) {
        setToastMessage(`⚠️ IMAP 删除失败: ${data.error}`);
        setShowToast(true);
    }
}
```
...
- `worker/imap-worker.ts` - `AccountWatcher.deleteEmail` 方法
- `app/api/messages/[id]/route.ts` - DELETE 端点
- `app/page.tsx` - `deleteEmail` 函数

### 优先级

1. **立即修复**：归档邮件删除问题（方案 A）
2. **立即修复**：前端错误处理
3. **可选**：WebSocket 反馈机制
