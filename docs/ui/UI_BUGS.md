...
**问题描述**：
在桌面三栏布局中，当未选中任何邮件时，右侧邮件详情栏显示为空白区域，缺少默认的占位框架，视觉上显得不协调且用户体验不佳。

**解决方案**：
经检查，`DetailEmptyState` 组件和对应样式已存在于代码库中：

- ✅ 组件已存在：`app/components/DetailEmptyState.tsx`
- ✅ 组件已导入：`app/page.tsx` 第 13 行
- ✅ 组件已使用：`app/page.tsx` 第 1273-1291 行
- ✅ CSS 样式已存在：`app/globals.css` 第 2624-2711 行
...
- `app/components/DetailEmptyState.tsx`
- `app/page.tsx`
- `app/globals.css`

**状态**：🟢 已修复 (2026-01-13 23:00)

**补充说明**：此功能在之前的桌面三栏布局实现中已经完成，本次只是确认和记录。
...
**问题描述**：
左侧边栏的账号列表显示多个账号时，当账号数量超过可视区域高度时，无法通过滚动查看底部的账号，导致部分账号无法访问。

**根本原因**：
Flexbox 容器的子元素需要 `min-height: 0` 才能正常滚动。`.sidebar-accounts` 虽然有 `flex: 1` 和 `overflow-y: auto`，但缺少 `min-height: 0`，导致滚动失效。

**解决方案**：
在 `app/globals.css` 的 `.sidebar-accounts` 样式中添加 `min-height: 0`：

```css
.sidebar-accounts {
    flex: 1;
    min-height: 0; /* Fix: 确保 flex 容器内滚动生效 */
    overflow-y: auto;
    overflow-x: hidden;
    padding: 16px;
...
**技术原理**：
在 Flexbox 布局中，子元素默认的 `min-height: auto` 会防止其收缩到小于内容高度。设置 `min-height: 0` 可以解除这个约束，使 `overflow-y: auto` 生效。

**相关文件**：

- `app/globals.css` 第 925-933 行

**状态**：🟢 已修复 (2026-01-13 23:00)

---

### Bug #34: 全选复选框跨上下文累积选中项（M7）
...
**问题描述**：
`MessageList.tsx` (lines 215-233) 全选复选框未先清空非当前列表的选中项。切换筛选/搜索/文件夹后点击"全选"，批量操作会同时作用于之前隐藏的邮件，"已选"计数跨上下文累积。

**预期行为**：全选时应重置为当前 `filteredEmails`，或在上下文变化时自动清空选中。

**相关文件**：`app/components/MessageList.tsx` (lines 215-233)
**状态**：🟢 已修复 (2026-01-13 22:25)

**解决方案**：全选前先调用 `clearSelection()` 清空选中集

---

### Bug #33: 搜索请求缺少并发防护（M7）
...
**问题描述**：
`page.tsx` (lines 526-594) 的 `loadEmails()` 没有 AbortController/请求令牌；用户快速输入时，先发出的旧查询响应可能后到并覆盖新查询，界面与搜索词不一致。

**预期行为**：搜索请求应添加中断或"最新请求"校验再落地状态。

**相关文件**：`app/page.tsx` (loadEmails 函数)
**状态**：🟢 已修复 (2026-01-13 22:25)

**解决方案**：添加 `loadEmailsVersionRef` 版本号，`setEmails` 前校验版本号是否为最新

---

### Bug #32: 搜索防抖计时器未随上下文切换清理（M7）
...
**问题描述**：
`page.tsx` (lines 685-692) 的搜索防抖 useEffect 延迟 `loadEmails()` 仍会按旧的 `activeFolder` 触发并覆写最新结果。切换文件夹后列表可能又回到旧数据。

**预期行为**：把依赖扩展到 `activeFolder`/`selected` 或在这些状态变化时取消未决的搜索定时/请求。

**相关文件**：`app/page.tsx` (searchQuery useEffect)
**状态**：🟢 已修复 (2026-01-13 22:25)

**解决方案**：切换账号/文件夹时调用 `setSearchQuery('')` 清空搜索词

---

### Bug #31: 移动端筛选工具栏布局问题（M7）
...
1. 全选复选框和排序按钮之间有大量空白，未充分利用空间
2. 筛选芯片被截断（如“附件”显示为“附”）
3. 筛选芯片水平滚动不明显，用户可能不知道可以滚动
4. 整体布局占用空间较大，减少了邮件列表可视区域

**相关文件**：

- `app/globals.css` (list-toolbar-enhanced 样式)
- `app/components/MessageList.tsx` (筛选工具栏)

**状态**：� 已修复 (2026-01-13 21:55)

**解决方案**：优化移动端 CSS - `flex-wrap:wrap` + 搜索框独占行 + 芯片 `white-space:nowrap` 防截断 + 渐变遮罩提示可滚动

---

### Bug #29: 轮询刷新会重置邮件星标状态（M7）
...
**问题描述**：
当 WebSocket 轮询完成后触发的 `loadEmails()` 刷新会覆盖用户在本地设置的星标状态。星标筛选显示 0 条结果，表明 API 返回的 `starred` 字段被重置或丢失。

**现象**：

- 筛选芯片显示"星标 0"
- 尽管用户已为邮件添加星标，刷新后星标状态消失

**可能原因**：

1. 数据库中没有正确持久化 `starred` 字段
2. `/api/messages` 返回数据时 `starred` 字段为空或 false
3. `loadEmails()` 刷新时丢弃了本地临时状态

**相关文件**：

- `app/page.tsx` (loadEmails 函数)
- `app/api/messages/route.ts` (starred 字段返回)

**状态**：🟢 已修复 (2026-01-13 21:45)

---

### Bug #30: Star API 请求冗余（双重请求）（M7）
...
1. `/api/messages/{id}/star` (200 OK)
2. `/api/messages/{id}/star/` (带尾斜杠，可能导致重复调用)

**现象**：

- DevTools 网络面板显示 `star` 和 `star/` 两个请求
- 可能导致星标状态被重复切换（toggle 两次 = 无效果）

**可能原因**：

1. 前端代码中 URL 拼接有问题
2. Next.js trailingSlash 配置导致重定向

**相关文件**：

- `app/page.tsx` (onStar 调用)
- `app/api/messages/[id]/star/route.ts`
- `next.config.ts` (trailingSlash 配置)
